...
public class PainelDao extends Dao{
    ...
    public List<painel_ant> lista_senhas_chamadas() {
        ...
        try {
            ...
            String sql = "SELECT max(s.id_senha) senha, f.id_fila fila FROM tab_senhas s inner join tab_fila f on f.id_usuario = s.id_usuario WHERE s.id_usuario is not null and s.data_senha = curdate() group by f.id_fila";
   ...
   public painel_atu lista_senhas_anteriores() {
        ...
        try {
            ...
            sql = "SELECT max(id_senha) senha, status_atendimento from tab_senhas WHERE status_atendimento in (\"Chamando\", \"Em Atendimento\", \"Atendido\") and data_senha = curdate()" ;
            ...
            while (result.next()){
               ...
               atu.setStatus_atendimento(result.getString("status_atendimento"));
            }
